...
//	}
	...
	public PnrTicketNo setPnrandTkt() {
	    ...
	    long tktno = pt0.getTicketNumber() + 1;
	    pt0.setTicketNumber(tktno);
	    
	    // Update the PNR with the new ticket number
	    String pnr = pt0.getPnr();
	    pnr = pnr.replaceAll("-", ""); // Remove hyphens
	    pnr = pnr.substring(0, 3) + "-" + tktno; // Concatenate with the new ticket number
	    pt0.setPnr(pnr);
